...
export function ListingScreen({ navigation, route }: Props) {
  ...
  const subtitleLabel = useMemo(
    () => [spaceTypeLabel, areaOnlyLabel, distanceLabel && `${distanceLabel} away`]
      .filter(Boolean)
      .join(" · "),
    [spaceTypeLabel, areaOnlyLabel, distanceLabel]
  );
  ...
  // Only show a monthly cross-sell when the space genuinely offers one and the
  // driver isn't already in the monthly lane.
  const showMonthlyOffer =
    !isMonthly && !!listing && Number(listing.price_per_month) > 0;
              ...
              <View style={styles.sheet}>
                ...
                >
                  <View style={styles.factIcon}>
                    <Star size={22} color={GREEN} fill={GREEN} strokeWidth={0} />
                  </View>
                  ...
                  {/* Grey, not green: the row's text already carries the brand
                      colour, and a green chevron beside it doubles the accent
                      on a single row. */}
                  {hasReviews ? <ChevronRight size={18} color={FG_SUBTLE} strokeWidth={2.2} /> : null}
                </Pressable>
                ...
                <View style={styles.factRow}>
                  ...
                  <View style={styles.factIcon}>
                    <Info size={22} color={FG_SUBTLE} strokeWidth={1.6} />
                  </View>
                  <View style={styles.factBody}>
                ...
                {listing.hostName?.trim() ? (
                  <View style={styles.factRow}>
                    <View style={styles.factIcon}>
                      <UserRound size={22} color={FG_SUBTLE} strokeWidth={1.6} />
                    </View>
                ...
                {/* ── Time selection ──────────────────────────────────────────
                    Sits in the masthead rather than down on the ground: it is
                    the one control on this screen that changes the price, so it
                    belongs beside the facts, not below them. */}
                <View style={styles.timePicker}>
                  ...
                  <View style={styles.timePickerFooter}>
                    <Clock size={14} color={FG_SUBTLE} strokeWidth={2} />
                    <Text style={styles.timePickerDuration}>
                      {isMonthly ? "1 month" : priceSummary?.durationLabel ?? ""}
                    </Text>
                    <Text style={styles.timePickerHint}>Reserved instantly</Text>
                  </View>
                </View>
              ...
              <View style={[styles.ground, { paddingBottom: bottomBarSpacer }]}>
                <View style={styles.policyBlock}>
                  <Text style={styles.policyText}>
                    Exact address and access details are shared the moment you book. See our{" "}
                ...
                {showMonthlyOffer ? (
                  ...
                  >
                    <View style={styles.offerCardCopy}>
                      <Text style={styles.offerCardTitle}>Park here monthly</Text>
                      <Text style={styles.offerCardBody}>
                        {`This host offers a monthly space from €${formatPriceValue(monthlyPrice)}.`}
                      </Text>
                    </View>
                    <View style={styles.offerCardIcon}>
                      <CalendarDays size={17} color={GREEN} strokeWidth={2} />
                    </View>
                  </Pressable>
                ) : null}
...
const styles = StyleSheet.create({
  ...
  factRow: {
    ...
    minHeight: 34, paddingVertical: 5,
  },
  ...
  factRating: { fontFamily: "PlusJakartaSans-SemiBold", fontSize: 16, color: GREEN },
  ...
  factText: { fontFamily: "PlusJakartaSans-Regular", fontSize: 16, color: FG },
  ...
  timePickerHalf: { flex: 1, paddingVertical: 12, paddingHorizontal: 14 },
  timePickerRule: { width: 1, backgroundColor: EDGE },
  ...
  timePickerLabel: {
    ...
    fontSize: 11, letterSpacing: 0.6, textTransform: "uppercase", color: GREEN,
  },
  timePickerValueRow: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 4 },
  timePickerValue: {
    fontFamily: "PlusJakartaSans-ExtraBold",
  ...
  timePickerDate: {
    fontFamily: "PlusJakartaSans-Regular", fontSize: 12, color: FG_SUBTLE, marginTop: 2,
  },
  timePickerFooter: {
  ...
  timePickerDuration: { fontFamily: "PlusJakartaSans-SemiBold", fontSize: 13, color: FG },
  timePickerHint: {
    marginLeft: "auto",
  ...
  policyBlock: { paddingHorizontal: 16, paddingTop: 16, paddingBottom: 14 },
  policyText: { fontFamily: "PlusJakartaSans-Regular", fontSize: 13, lineHeight: 19, color: FG_2 },
  policyLink: { fontFamily: "PlusJakartaSans-SemiBold", color: GREEN },
  ...
  offerCardCopy: { flex: 1, minWidth: 0 },
  offerCardTitle: { fontFamily: "PlusJakartaSans-Bold", fontSize: 14, color: FG },
  offerCardBody: {
    fontFamily: "PlusJakartaSans-Regular", fontSize: 12, lineHeight: 17,
    color: FG_2, marginTop: 3,
  },
  offerCardIcon: {
