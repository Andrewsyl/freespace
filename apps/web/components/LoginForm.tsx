...
/**
 * Shared sign-in UI used by both the /login page and the global login modal,
 * so the two stay visually identical. Navigation after a successful sign-in is
 * delegated to the caller via `onSuccess` (the page pushes a route; the modal
 * refreshes in place or follows `next`). `onNavigate` lets a host modal close
 * itself when a footer link (sign up / reset password) is followed.
 */
export function LoginForm({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      handleSuccess(signedInUser?.name);
    } catch {
  ...
  return (
    <div className="mx-auto w-full max-w-sm">
      ...
      <div className="mt-8 space-y-3 text-center text-[14px] text-slate-600">
        <p>
          No account?{" "}
          <Link href="/signup" onClick={onNavigate} className="font-semibold text-brand-700">
            Sign up
          </Link>
        </p>
        <button
